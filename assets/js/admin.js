(function ($) {
    ...
    const config = window.wcversarioAdmin || null;
    ...
    const selectors = {
        modal: '#wcversario-modal',
        content: '#wcversario-modal .wcversario-modal__content',
        title: '#wcversario-modal .wcversario-modal__title',
    };
    ...
    function renderLoading() {
        ...
        $(selectors.content).html('<div class="wcversario-loading">' + (i18n.loading || 'Loading…') + '</div>');
    }
    ...
    function loadProductData() {
        $.post(
            config.ajaxUrl,
            {
                action: 'wcversario_get_product_data',
                nonce: config.nonce,
    ...
    function renderError(message) {
        ...
        $(selectors.content).html('<div class="wcversario-loading wcversario-error">' + errorText + '</div>');
    }
    ...
    function renderModalContent() {
        ...
        const selectsHtml = [
            '<div class="wcversario-step">',
            '  <h3>' + (i18n.selectAttributes || '') + '</h3>',
            '  <div class="wcversario-selects">',
            '      <label>' + (i18n.firstAttribute || '') + '<br><select class="wcversario-attr-select" data-role="first"><option value="">' + (i18n.selectPlaceholder || '') + '</option>' + attributeOptions + '</select></label>',
            '      <label>' + (i18n.secondAttribute || '') + '<br><select class="wcversario-attr-select" data-role="second"><option value="">' + (i18n.selectPlaceholder || '') + '</option>' + attributeOptions + '</select></label>',
            '  </div>',
            '</div>'
        ].join('');

        const termsHtml = '<div class="wcversario-step wcversario-terms" id="wcversario-terms"></div>';
        const tableHtml = '<div class="wcversario-step" id="wcversario-table"></div>';
        const changesHtml = '<div class="wcversario-step wcversario-changes" id="wcversario-changes"></div>';
        const footerHtml = '<div class="wcversario-footer"><button type="button" class="button button-primary" id="wcversario-confirm">' + (i18n.confirmChanges || '') + '</button></div>';
    ...
    function restoreSelectState() {
        if (state.selectedFirst) {
            $(selectors.content)
                .find('.wcversario-attr-select[data-role="first"]').val(state.selectedFirst);
        }
        if (state.selectedSecond) {
            $(selectors.content)
                .find('.wcversario-attr-select[data-role="second"]').val(state.selectedSecond);
        }
    }
    ...
    function renderTerms() {
        const container = $(selectors.content).find('#wcversario-terms');
        container.empty();
    ...
    function renderTermGroup(role, attribute, selectedTerms) {
        ...
        const itemsHtml = terms
            .map((term) => {
                ...
                return '<label><input type="checkbox" class="wcversario-term" data-role="' + role + '" value="' + term.slug + '" ' + checked + '> ' + term.name + '</label>';
            })
            .join('');

        return (
            '<div class="wcversario-terms__group">' +
            '  <h3>' + attribute.label + '</h3>' +
            '  <div class="wcversario-terms__items">' + itemsHtml + '</div>' +
            '</div>'
        );
    }

    function renderTable() {
        const container = $(selectors.content).find('#wcversario-table');
        container.empty();
        ...
        const rows = state.selectedSecondTerms.map((secondTermSlug) => {
            ...
            const cells = state.selectedFirstTerms.map((firstTermSlug) => {
                ...
                const inputId = 'wcversario-cell-' + firstTermSlug + '-' + secondTermSlug;
        ...
        const tableHtml = [
            '<div class="wcversario-table-wrapper">',
            '  <table class="wcversario-table">',
    ...
    function renderChanges() {
        const container = $(selectors.content).find('#wcversario-changes');
        container.empty();
    ...
    function handleConfirm() {
        ...
        const postData = {
            action: 'wcversario_save_variations',
            nonce: config.nonce,
            product_id: state.productId,
        ...
        const $button = $(selectors.content).find('#wcversario-confirm');
    ...
    function handleAttributeChange(role, value) {
        ...
        if (state.selectedFirst && state.selectedSecond && state.selectedFirst === state.selectedSecond) {
            if (role === 'first') {
                state.selectedSecond = null;
                state.selectedSecondTerms = [];
                $(selectors.content).find('.wcversario-attr-select[data-role="second"]').val('');
            } else {
                state.selectedFirst = null;
                state.selectedFirstTerms = [];
                $(selectors.content).find('.wcversario-attr-select[data-role="first"]').val('');
            }
    ...
    $(document).on('click', '.wcversario-open', function (event) {
    ...
    $(document).on('click', '.wcversario-modal__close, .wcversario-modal__backdrop', function (event) {
        event.preventDefault();
        closeModal();
    });

    $(document).on('change', '.wcversario-attr-select', function () {
        const role = $(this).data('role');
        const value = $(this).val();
        handleAttributeChange(role, value);
    });

    $(document).on('change', '.wcversario-term', function () {
        const role = $(this).data('role');
        const slug = $(this).val();
    ...
    $(document).on('change', '#wcversario-table input[type="checkbox"]', function () {
        const key = $(this).data('key');
        const checked = $(this).is(':checked');
        handleCellToggle(key, checked);
    });

    $(document).on('click', '#wcversario-confirm', function (event) {
        event.preventDefault();
        const firstAttr = getAttributeByName(state.selectedFirst);
        const secondAttr = getAttributeByName(state.selectedSecond);
